...
struct Node {
    ...
    void erase(int x) {
        v.erase(v.find_by_order(v.order_of_key(x)));
    }
};
// 30
struct MergeTree {
...
// 11
    void update(int l, int r, int node, int idx, int oldV, int newV) { // O(log^2(n))
        ...
        if (idx <= mid)
            update(l, mid, LNodeIDX, idx, oldV, newV);
        else
            update(mid + 1, r, RNodeIDX, idx, oldV, newV);
    }
// 11
    int query(int l, int r, int node, int Lx, int Rx, int v) { // O(log^2(n))
        ...
        if (Lx <= l and r <= Rx)
            return (int) Tree[node].v.order_of_key(v + 1);
...
// 3
    void update(int idx, int oldV, int newV) {
        update(0, SegSize - 1, 0, idx, oldV, newV);
    }
// 3
    int query(int l, int r, int v) {
        return query(0, SegSize - 1, 0, l, r, v);
    }
};
// 24
signed  main() {
