...
struct DSU {
    ...
    void snapshot() {
        checkPoint.push_back(update.size());
    }
...
signed main() {
    ...
    for (auto &[x, start]: query) {
        update(0, 0, Q - 1, start, Q - 1, x);
    }

    for (auto &[u, v, start, end]: add) {
        update(0, 0, Q - 1, start, end, {u, v});
    }
