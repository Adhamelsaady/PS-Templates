 ...
 *
 * The main modulus is 998244353 = 119 * 2^23 + 1 and root=62. NTT is the
 * exact modular analogue of FFT and avoids floating-point precision errors.
 *
 * The file also contains polynomial shifting P(x)->P(x+k) and CRT helpers for
 * combining results from several NTT-friendly moduli.
 *
 * WARNING: this source is a collection of snippets and currently contains a
 * second declaration named mod later in the file. Clean up the duplicate names
 * before compiling it as one translation unit.
 */
#include<bits/stdc++.h>
using namespace std;
...
// Fast modular exponentiation under the fixed NTT modulus.
...
// Find a primitive root by testing the prime factors of phi(mod).
...
int modpow(int b, int e, int m) {
    ...
    return ans;
// In-place iterative Number Theoretic Transform. This transform is exact modulo mod.
}
...
// Polynomial convolution using NTT.
}
...
// Compute coefficients of P(x+k) using factorials and one convolution.
}
...
// Combine two congruences using the Chinese Remainder Theorem.
...
// Combine three NTT-friendly residues, then reduce the reconstructed value to desired_mod.
