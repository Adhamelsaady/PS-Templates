 ...
 *
 * What this file does:
 * Finds x such that x^k = a (mod p), usually by converting the problem to a discrete logarithm.
 *
 * Typical problem to recognize:
 * A primitive root converts multiplication in a finite field into exponent arithmetic.
 *
 * Complexity / constraints:
 * Requires the assumptions stated in the source, usually prime modulus and suitable gcd conditions.
 *
 * Main variables:
 * a = target residue; k = exponent; p = prime modulus; x = root.
 *
...
// Function: cnt(ll a, ll base,ll p).
// Purpose: see the file header for the problem this helper solves; parameter names above are the contract.
...
// Function: gcd(ll a,ll b).
// Purpose: see the file header for the problem this helper solves; parameter names above are the contract.
ll gcd(ll a,ll b) {
  return a == 0 ? b : gcd(b % a, a);
}
// Function: peth_root(ll a, ll p, int e, ll mod).
// Purpose: see the file header for the problem this helper solves; parameter names above are the contract.
...
// 0^0 = 1 mod p
// Complexity: O(p ^ (1 / 4))
// Function: discrete_root(ll k, ll a, ll p).
// Purpose: see the file header for the problem this helper solves; parameter names above are the contract.
