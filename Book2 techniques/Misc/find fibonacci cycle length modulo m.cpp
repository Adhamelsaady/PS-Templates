// Pisano period of Fibonacci modulo m
// Problem: Compute the length of the repeating Fibonacci sequence modulo m. The algorithm factors m using Miller-Rabin + Pollard-Rho, computes the period for each prime power, then takes their LCM.
// Use: Input t followed by moduli m. Output pi(m), the smallest positive T with F(T)=0 and F(T+1)=1 modulo m.
// Variables: Key variables: Matrix.m=2x2 matrix entries; P=small sieve limit; seq=Pollard-Rho collision buffer; primes/spf=small-prime sieve data; r/s=decomposition of n-1; factors=prime factors; candidate=period upper bound for a prime; period_p=period modulo p; e=prime exponent; period_pp=period modulo p^e; overall=LCM of prime-power periods.
// Idea: Terminology: Pisano period is the cycle length of Fibonacci numbers modulo m. Miller-Rabin tests primality; Pollard-Rho factors large integers; LCM combines coprime/prime-power periods.
// Complexity: Complexity: factorization dominates and is probabilistic/advanced; matrix/Fibonacci exponentiation is O(log m) per test. Memory is O(P) for the factorization helper.
